...
def get_connection_settings():
    """
    Возвращает параметры подключения к PostgreSQL из переменных окружения.
    Ничего не читаем из creds.db — это требование преподавателя.
    """
    try:
        settings = {
...
def load_data():
    ...
    # Загружаем данные из локального Data_clean.parquet, если нет, то скачиваем CSV из Google Drive и делает приведение типов.
    ...
    try:
        ...
        # Приведение типов (как в data_loader.py)
...
def upload_to_database(df: pd.DataFrame, db_settings: dict, table_name: str):
    ...
    Подключение формируется через URL.create.
    """
    try:
        # Безопасная сборка URL (экранирует спецсимволы в пароле)
        url = URL.create(
...
def main():
    ...
    # Временная отладка: смотрим, что реально подхватилось из .env
    print("Проверяем, что .env подгружен корректно:")
    print("HOST repr:", repr(os.getenv("DB_HOST")))
    print("PORT repr:", repr(os.getenv("DB_PORT")))
    print("DB   repr:", repr(os.getenv("DB_NAME")))
    print("USER repr:", repr(os.getenv("DB_USER")))
    pwd = os.getenv("DB_PASSWORD")
    print("PASS len:", 0 if pwd is None else len(pwd), "repr:", repr(pwd))
    print("-" * 50)
    ...
    if engine is not None:
        ...
        try:
            count = pd.read_sql('SELECT COUNT(*) FROM public."khabibullina"', engine).iloc[0, 0]
            print(f"\nКоличество строк в public.khabibullina: {count}")
        except Exception as e:
        ...
        if ok:
            ...
            print("Данные записаны, но проверка не завершилась корректно.")
    else:
