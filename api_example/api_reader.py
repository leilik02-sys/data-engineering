...
)  # Создаем папку для сохранения JSON
...
def to_dataframe_otus_style(
    records: list[dict],
) -> pd.DataFrame:  # Преобразуем список JSON-объектов из api в DataFrame
    if not records:
        return pd.DataFrame()

    df = pd.json_normalize(
        records, sep="."
    )  # Изменяем вложенные поля в формат ClassJob.NameEnglish
...
if __name__ == "__main__":
    ...
    df = to_dataframe_otus_style(result)  # Преобразуем JSON в DataFrame
